...
namespace {
  ...
  TEST_F(SimplePickerTest, testpicker) {
    ...
    size_t x = sp(data2,data2+sizeof(data2)/sizeof(double),zerocross.begin(),2);
    ASSERT_NEAR(zerocross[0],5.,1e-10);
    x = sp(data2,data2+sizeof(data2)/sizeof(double),zerocross.begin(),4);
    ASSERT_NEAR(zerocross[0],4.6666,1e-4);
    x = sp(data2,data2+sizeof(data2)/sizeof(double),zerocross.begin(),6);
    ASSERT_NEAR(zerocross[0],4.5,1e-4);
    std::cout << x << std::endl;


    double data3[] = {0.,1.,2.,3.,5.,6.,5.,4.,3.,2.,1.};
    x = sp(data3,data3+sizeof(data3)/sizeof(double),zerocross.begin(),2);
    x = sp(data3,data3+sizeof(data3)/sizeof(double),zerocross.begin(),4);
    ASSERT_NEAR(zerocross[0],5.3333,1e-4);
    x = sp(data3,data3+sizeof(data3)/sizeof(double),zerocross.begin(),6);
